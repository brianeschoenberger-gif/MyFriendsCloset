...
export function AddItemModal({ close, save }: { close: () => void; save: (item: ClosetItem) => void }) {
  ...
  async function selectImage(file?: File) {
    ...
    if (file.size > 5_000_000) { setError('Choose an image under 5 MB for local demo storage.'); return }
    try { setImageData(await fileToDataUrl(file)); setError('') } catch { setError('We could not read that photo. Try another image.') }
  }
  ...
  return <div className="modal-backdrop"><form className="add-modal" aria-label="Add clothing item" onSubmit={(event) => {
    ...
    save({ id: Date.now(), ownerId: 'owner', owner: 'Brian', imageData, name: String(data.get('name')), category: String(data.get('category')), color: String(data.get('color')), size: String(data.get('size')), brand: String(data.get('brand')), visibility, occasionTags: String(data.get('tags')).split(',').map((tag) => tag.trim()).filter(Boolean), notes: String(data.get('notes')), glyph: 'PHOTO', createdAt: now, updatedAt: now })
  }}><button type="button" className="modal-close" onClick={close} aria-label="Close add item"><X /></button><p className="eyebrow">A new closet favorite</p><h2>Add an item</h2><label className={`upload-zone ${imageData ? 'has-preview' : ''}`}>{imageData ? <img src={imageData} alt="New item preview" /> : <><Camera /><strong>Take or choose a photo</strong><span>Camera opens on supported phones</span></>}<input className="file-input" name="photo" type="file" accept="image/*" capture="environment" onChange={(event) => selectImage(event.target.files?.[0])} /></label>{error && <p className="form-error" role="alert">{error}</p>}<label>Item name<input name="name" placeholder="e.g. The perfect black dress" required /></label><div className="form-grid"><label>Category<select name="category"><option>Dresses</option><option>Jackets</option><option>Shoes</option><option>Accessories</option><option>Tops</option><option>Bottoms</option></select></label><label>Size<input name="size" defaultValue="M" required /></label><label>Brand<input name="brand" placeholder="Optional" /></label><label>Color<input name="color" placeholder="Black" required /></label></div><label>Occasion tags<input name="tags" placeholder="Dinner, vacation, work" /></label><label>Notes<textarea name="notes" placeholder="Fit notes, care instructions, or anything friends should know" /></label><fieldset><legend>Who can see it?</legend><div className="visibility-options">{(['Private', 'Friends', 'Borrowable'] as Visibility[]).map((option) => <button type="button" key={option} className={visibility === option ? 'selected' : ''} onClick={() => setVisibility(option)}>{option}</button>)}</div></fieldset><button className="primary-button full" type="submit">Save to my closet</button></form></div>
}

export function BorrowRequestModal({ item, requester, close, save }: { item: ClosetItem; requester: Identity; close: () => void; save: (request: BorrowRequest) => void }) {
  return <div className="modal-backdrop"><form className="request-modal add-modal" aria-label="Request to borrow" onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); const now = new Date().toISOString(); const startDate = String(data.get('startDate')); const endDate = String(data.get('endDate')); save({ id: Date.now(), itemId: item.id, ownerId: item.ownerId, requesterId: requester, requesterName: 'Alex', item: item.name, person: item.owner, dates: startDate && endDate ? `${startDate} to ${endDate}` : 'Dates flexible', direction: 'incoming', status: 'Pending', glyph: item.glyph, startDate, endDate, note: String(data.get('note')), createdAt: now, updatedAt: now }) }}><button type="button" className="modal-close" onClick={close} aria-label="Close borrow request"><X /></button><p className="eyebrow">Ask Brian</p><h2>Borrow {item.name}?</h2><div className="request-item-preview"><ItemVisual item={item} compact /><p>Share your dates and a quick note. Brian can approve or decline from owner mode.</p></div><div className="form-grid"><label>Start date<input name="startDate" type="date" /></label><label>End date<input name="endDate" type="date" /></label></div><label>Note<textarea name="note" placeholder="I would love this for Saturday's dinner!" required /></label><button className="primary-button full" type="submit">Send borrow request</button></form></div>
}
